...
export default function MindSpace() {
  ...
  if (screen === "auth") return <div style={styles.page}>{toast && <Toast>{toast}</Toast>}<div style={{ maxWidth: 520, margin: "45px auto" }}><Card><Buddy /><h1 style={styles.title}>{authMode === "login" ? "Добре дојде назад 👋" : "Да се запознаеме 🌱"}</h1>{authMode === "register" && <><Input placeholder="Име" value={auth.name} onChange={v => setAuth({ ...auth, name: v })} /><p style={styles.authHint}>Внеси име или прекар. Полето е задолжително и не смее да биде празно.</p></>}<Input placeholder="Е-пошта" value={auth.email} onChange={v => setAuth({ ...auth, email: v })} /><p style={styles.authHint}>Внеси валидна е-пошта. Може да користиш Gmail, Yahoo, Outlook, Hotmail или било која друга e-mail адреса. Полето е задолжително.</p><Input type="password" placeholder="Лозинка" value={auth.pass} onChange={v => setAuth({ ...auth, pass: v })} /><p style={styles.authHint}>Лозинката е задолжителна и мора да содржи најмалку 8 карактери.</p>{authMode === "register" && <><Input type="password" placeholder="Потврди лозинка" value={auth.pass2} onChange={v => setAuth({ ...auth, pass2: v })} /><p style={styles.authHint}>Повтори ја истата лозинка. Двете лозинки мора целосно да се совпаѓаат.</p></>}<Button onClick={authMode === "login" ? loginWithPassword : register}>{authMode === "login" ? "Најави се" : "Создај профил"}</Button>{authMode === "register" && <p style={styles.authHint}>Сите полиња се задолжителни за регистрација.</p>}<p style={styles.textSmall}>{authMode === "login" ? "Немаш профил? " : "Веќе имаш профил? "}<button style={styles.link} onClick={() => setAuthMode(authMode === "login" ? "register" : "login")}>{authMode === "login" ? "Регистрирај се" : "Најави се"}</button></p></Card></div></div>;
...
const styles = {
  ...
  text: { color: COLORS.muted, lineHeight: 1.55, fontSize: 15 }, textSmall: { color: COLORS.muted, textAlign: "center" }, authHint: { color: COLORS.muted, fontSize: 12, lineHeight: 1.4, marginTop: -4, marginBottom: 8, paddingLeft: 4 }, muted: { color: COLORS.muted, fontSize: 13, margin: 0 },
